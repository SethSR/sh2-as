...
use crate::parser::{LabelMap, LabelType};
...
impl From<Asm> for Item {
	fn from(asm: Asm) -> Self { Self::Asm(asm) }
}

impl From<Dir> for Item {
	fn from(dir: Dir) -> Self { Self::Asm(Asm::Dir(dir)) }
}
...
pub fn resolver(
	...
	let mut rep_offset = 0u32;
	for asm in &assembly {
		match asm {
			Asm::Dir(dir) => match dir {
				Dir::ConstImmByte(..) => addr += 1,
				Dir::ConstImmWord(..) => addr += 2,
				Dir::ConstImmLong(..) => addr += 4,
				Dir::ConstLabelLong(..) => addr += 4,
				Dir::Org(new_addr) => addr = *new_addr,
				Dir::Repeat(num) => rep_count = *num as u32,
			}
			Asm::Ins(ins) => match ins {}
		}
	}

	let mut section: Vec<Item> = assembly.into_iter()
		.map(Item::Asm)
		.collect();

	let mut new_section: Vec<Item> = Vec::with_capacity(section.len() + 128);
	...
	for (idx, item) in section.iter().enumerate() {
		...
		match item {
			Item::Asm(Asm::Dir(dir)) => match dir {
				Dir::ConstImmByte(imm) => {
					new_section.push(Item::Byte(*imm));
				}
				Dir::ConstImmWord(imm) => {
					new_section.push(Item::Word(*imm));
				}
				Dir::ConstImmLong(imm) => {
					new_section.push(Item::Long(*imm));
				}
				Dir::ConstLabelLong(lbl) => match labels[lbl] {
					LabelType::Unknown => new_section.push(Dir::ConstLabelLong(lbl.clone()).into()),
					LabelType::Label(addr) => {
				...
				Dir::Org(addr) => {
					new_section.push(Item::NewSection(*addr));
				}
				Dir::Repeat(num) => {
					new_section.push(Item::RepeatStart(*num));
				}
				Dir::EndRepeat => {
					new_section.push(Item::RepeatEnd);
				}
				Dir::BinInclude(file) => {
					eprintln!("open file and dump bytes");
				}
				Dir::Align(alignment) => {
					new_section.push(Item::Align(*alignment));
				}
				Dir::Label(lbl) => {
					if let LabelType::Unknown = labels[lbl] {
						new_section.push(Dir::Label(lbl.clone()).into());
					}
				}
			}

			Item::Asm(Asm::Ins(ins)) => match ins {
